...
class TaskCard(CardWidget, Ui_TaskCard):
    ...
    def __onTaskInited(self):
        ...
        if self.fileName and not self.autoCreated:
            with open("{}/NobleBlocks 记录文件".format(cfg.appPath), "a", encoding="utf-8") as f:
    ...
    def pauseTask(self):
        if self.status == "working":  # 暂停
            ...
            try:
                ...
                # 改变记录状态
                with open("{}/NobleBlocks 记录文件".format(cfg.appPath), "r", encoding="utf-8") as f:
                    _ = f.read()
                ...
                with open("{}/NobleBlocks 记录文件".format(cfg.appPath), "w", encoding="utf-8") as f:
                    f.write(_)
